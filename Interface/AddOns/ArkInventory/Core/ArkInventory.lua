...
ArkInventory.Global = { -- globals
	...
	Location = {
		...
		[ArkInventory.Const.Location.Keyring] = {
			...
			ClientCheck = ArkInventory.ClientCheck( 11101, 40200 ),
			Internal = "key",
			Name = ArkInventory.Localise["KEYRING"],
		...
		[ArkInventory.Const.Location.ReagentBank] = {
			...
			ClientCheck = ArkInventory.ClientCheck( ArkInventory.ENUM.EXPANSION.DRAENOR, 110199 ),
...
--			isOffline = true,
		...
		[ArkInventory.Const.Location.Pet] = {
			...
			ClientCheck = ArkInventory.ClientCheck( 30002 ),
			isAccount = true,
			Internal = "pet",
			Name = ArkInventory.Localise["PET"],
		...
		[ArkInventory.Const.Location.MountEquipment] = {
			...
			ClientCheck = ArkInventory.ClientCheck( 80200 ),
		...
		[ArkInventory.Const.Location.Toybox] = {
			...
			ClientCheck = ArkInventory.ClientCheck( 60200 ),
			isAccount = true,
			Internal = "toybox",
		...
		[ArkInventory.Const.Location.Heirloom] = {
			...
			ClientCheck = ArkInventory.ClientCheck( 60100 ),
			isAccount = true,
			Internal = "heirloom",
			Name = ArkInventory.Localise["HEIRLOOM"],
		...
		[ArkInventory.Const.Location.Void] = {
			...
			ClientCheck = ArkInventory.ClientCheck( ArkInventory.ENUM.EXPANSION.WARWITHIN, 110199 ), -- void storage was added around 4.4.0 but it doesnt look like theyre adding it back into classic.  so its only in the war within up until 11.2.0 (where it got removed from retail)
			Internal = "void",
			Name = ArkInventory.Localise["VOID_STORAGE"],
				...
				["**"] = { -- id
					...
					["location"] = {
						["**"] = { -- loc_id
							...
							["lastselected"] = { 
								 ["remember"] = 1, -- remember the last bag/tab that was selected and open to it next time the window is opened?  0 = never | 1 = per session | 2 = permanent
								 ["bag_id_window"] = 1, -- bag_id_window
							},
						},
						[ArkInventory.Const.Location.Bag] = {
							["notify"] = true,
...
function ArkInventory.TOCVersionFail( quiet )
	...
	if not quiet then
		--ArkInventory.OutputWarning( "This version is a work in progress to handle the changes in The War Within 11.2, and while it should continue to work in all other game clients you may encounter problems in those as well.\n\nIf you encounter any issues with ArkInventory, regardless of game client, then please let me know by creating a new issue at https://github.com/arkayenro/arkinventory/issues" )
	end
...
--			ArkInventory.OutputError( "code failure: TOC Interface value does not have the correct format 123456 or 123456.78" )
	...
	if ArkInventory.TOCVersionFail( ) then
		ArkInventory.OnDisable( )
		return
	end
	...
	-- reagent bag
	if ArkInventory.Global.Location[ArkInventory.Const.Location.ReagentBag].ClientCheck then

		ArkInventory.Util.MapAddBag( { blizzard_id = ArkInventory.ENUM.BAG.INDEX.REAGENTBAG, loc_id_window = ArkInventory.Const.Location.Bag, loc_id_storage = ArkInventory.Const.Location.ReagentBag } )

	end


	-- keyring
	if ArkInventory.Global.Location[ArkInventory.Const.Location.Keyring].ClientCheck then
	...
	-- mailbox
	if ArkInventory.Global.Location[ArkInventory.Const.Location.Mailbox].ClientCheck then
	...
	-- wearing
	if ArkInventory.Global.Location[ArkInventory.Const.Location.Wearing].ClientCheck then
	...
	-- mount equipment
	if ArkInventory.Global.Location[ArkInventory.Const.Location.MountEquipment].ClientCheck then
	...
	-- currency (account wide)
	if ArkInventory.Global.Location[ArkInventory.Const.Location.AccountCurrency].ClientCheck then
	...
	-- auctions
	if ArkInventory.Global.Location[ArkInventory.Const.Location.Auction].ClientCheck then
	...
	-- reputation (account wide)
	if ArkInventory.Global.Location[ArkInventory.Const.Location.AccountReputation].ClientCheck then
	...
	-- tradeskill
	if ArkInventory.Global.Location[ArkInventory.Const.Location.Tradeskill].ClientCheck then
	...
	if ArkInventory.Global.Location[ArkInventory.Const.Location.TradeskillEquipment].ClientCheck then
...
--		{ "PLAYER_ACCOUNT_BANK_TAB_SLOTS_CHANGED", "EVENT_ARKINV_BANK_SLOT", ArkInventory.ENUM.EXPANSION.WARWITHIN },
		...
		{ "GUILDBANKBAGSLOTS_CHANGED", "EVENT_ARKINV_VAULT_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Vault].ClientCheck },
		{ "GUILDBANKLOG_UPDATE", "EVENT_ARKINV_VAULT_LOG", ArkInventory.Global.Location[ArkInventory.Const.Location.Vault].ClientCheck },
		{ "GUILDBANK_ITEM_LOCK_CHANGED", "EVENT_ARKINV_VAULT_LOCK", ArkInventory.Global.Location[ArkInventory.Const.Location.Vault].ClientCheck },
		{ "GUILDBANK_UPDATE_MONEY", "EVENT_ARKINV_VAULT_MONEY", ArkInventory.Global.Location[ArkInventory.Const.Location.Vault].ClientCheck },
		{ "GUILDBANK_UPDATE_TABS", "EVENT_ARKINV_VAULT_TABS_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Vault].ClientCheck },
		{ "GUILDBANK_UPDATE_TEXT", "EVENT_ARKINV_VAULT_INFO", ArkInventory.Global.Location[ArkInventory.Const.Location.Vault].ClientCheck },
...
--		{ "BANK_TAB_SETTINGS_UPDATED", "EVENT_ARKINV_", ArkInventory.ENUM.EXPANSION.WARWITHIN }, 0=CHAR, 1=GUILD, 2=ACCOUNT
		...
		{ "HEIRLOOMS_UPDATED", "EVENT_ARKINV_COLLECTION_HEIRLOOM_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Heirloom].ClientCheck },
...
--		{ "TRADE_SKILL_CURRENCY_REWARD_RESULT", "EVENT_ARKINV_ITEM_DATA_LOAD_RESULT" },
		...
		{ "CURRENCY_DISPLAY_UPDATE", "EVENT_ARKINV_COLLECTION_CURRENCY_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Currency].ClientCheck },
		...
		{ "MOUNT_EQUIPMENT_APPLY_RESULT", "EVENT_ARKINV_COLLECTION_MOUNT_EQUIPMENT_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.MountEquipment].ClientCheck },
--		{ "MOUNT_JOURNAL_SEARCH_UPDATED", "" },
--		{ "MOUNT_JOURNAL_USABILITY_CHANGED", "" },
		{ "NEW_MOUNT_ADDED", "EVENT_ARKINV_COLLECTION_MOUNT_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Mount].ClientCheck },
		{ "UNIT_AURA", "EVENT_ARKINV_UNIT_AURA", ArkInventory.Global.Location[ArkInventory.Const.Location.Mount].ClientCheck },
--		{ "PLAYER_CAN_GLIDE_CHANGED", "EVENT_ARKINV_PLAYER_CAN_GLIDE_CHANGED", ArkInventory.Global.Location[ArkInventory.Const.Location.Mount].ClientCheck },

		{ "BATTLE_PET_CURSOR_CLEAR", "EVENT_ARKINV_COLLECTION_PET_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Pet].ClientCheck },
--		{ "CHAT_MSG_PET_BATTLE_COMBAT_LOG", "EVENT_ARKINV_COLLECTION_PET_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Pet].ClientCheck },
--		{ "CHAT_MSG_PET_BATTLE_INFO", "EVENT_ARKINV_COLLECTION_PET_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Pet].ClientCheck },
--		{ "CHAT_MSG_PET_INFO", "EVENT_ARKINV_COLLECTION_PET_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Pet].ClientCheck },
		{ "COMPANION_LEARNED", "EVENT_ARKINV_COLLECTION_MOUNT_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Pet].ClientCheck },
		{ "COMPANION_UNLEARNED", "EVENT_ARKINV_COLLECTION_MOUNT_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Pet].ClientCheck },
--		{ "COMPANION_UPDATE", "EVENT_ARKINV_COLLECTION_MOUNT_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Pet].ClientCheck }, -- do i really need this? it triggers when other people mount/dismount as well
		{ "NEW_PET_ADDED", "EVENT_ARKINV_COLLECTION_PET_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Pet].ClientCheck },
		{ "PET_BATTLE_CLOSE", "EVENT_ARKINV_COLLECTION_PET_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Pet].ClientCheck },
		{ "PET_BATTLE_LEVEL_CHANGED", "EVENT_ARKINV_COLLECTION_PET_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Pet].ClientCheck },
		{ "PET_BATTLE_OPENING_DONE", "EVENT_ARKINV_BATTLEPET_OPENING_DONE", ArkInventory.Global.Location[ArkInventory.Const.Location.Pet].ClientCheck },
--		{ "PET_BATTLE_OVER", "EVENT_ARKINV_COLLECTION_PET_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Pet].ClientCheck },
--		{ "PET_BATTLE_PET_CHANGED", "EVENT_ARKINV_COLLECTION_PET_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Pet].ClientCheck },
--		{ "PET_BATTLE_PET_ROUND_RESULTS", "EVENT_ARKINV_COLLECTION_PET_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Pet].ClientCheck },
		{ "PET_BATTLE_QUEUE_STATUS", "EVENT_ARKINV_COLLECTION_PET_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Pet].ClientCheck },
--		{ "PET_BATTLE_XP_CHANGED", "EVENT_ARKINV_COLLECTION_PET_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Pet].ClientCheck },
		{ "PET_JOURNAL_LIST_UPDATE", "EVENT_ARKINV_COLLECTION_PET_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Pet].ClientCheck },
		{ "PET_JOURNAL_PET_DELETED", "EVENT_ARKINV_COLLECTION_PET_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Pet].ClientCheck },
		{ "PET_JOURNAL_PET_RESTORED", "EVENT_ARKINV_COLLECTION_PET_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Pet].ClientCheck },
		{ "PET_JOURNAL_PET_REVOKED", "EVENT_ARKINV_COLLECTION_PET_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Pet].ClientCheck },
		{ "PET_JOURNAL_PETS_HEALED", "EVENT_ARKINV_COLLECTION_PET_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Pet].ClientCheck },
...
--		{ "QUEST_LOG_UPDATE", "" }, -- triggers too often to be usable
		...
		{ "NEW_TOY_ADDED", "EVENT_ARKINV_COLLECTION_TOYBOX_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Toybox].ClientCheck },
		{ "TOYS_UPDATED", "EVENT_ARKINV_COLLECTION_TOYBOX_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Toybox].ClientCheck },
...
--		{ "VOID_STORAGE_CLOSE", "EVENT_ARKINV_VOID_LEAVE", ArkInventory.ClientCheck( ArkInventory.ENUM.EXPANSION.PANDARIA, ArkInventory.ENUM.EXPANSION.SHADOWLANDS ) }, -- void storage not implemented in cata beta yet so moved to pandaria temporarily
		...
		{ "VOID_STORAGE_UPDATE", "EVENT_ARKINV_VOID_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Void].ClientCheck },
		{ "VOID_STORAGE_CONTENTS_UPDATE", "EVENT_ARKINV_VOID_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Void].ClientCheck },
		{ "VOID_STORAGE_DEPOSIT_UPDATE", "EVENT_ARKINV_VOID_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Void].ClientCheck },
		{ "VOID_TRANSFER_DONE", "EVENT_ARKINV_VOID_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Void].ClientCheck },
--		{ "VOID_DEPOSIT_WARNING", "EVENT_ARKINV_VOID_UPDATE", ArkInventory.Global.Location[ArkInventory.Const.Location.Void].ClientCheck },
...
--	ArkInventory.Output( "Frame_Main_Draw_Threaded( ", frame:GetName( ), " ) drawstate[", ArkInventory.Global.Location[loc_id].drawState, "]" ) --, framelevel[", frame:GetFrameLevel( ), "]" )
	...
	if loc_id == ArkInventory.Const.Location.Vault then
		...
		local obj = _G[string.format( "%s%s", frame:GetName( ), ArkInventory.Const.Frame.Container.Name )]
		obj:Hide( )
	...
	if ArkInventory.Global.Location[loc_id].drawState <= ArkInventory.Const.Window.Draw.Restart then
		...
		ArkInventory.MediaFrameDefaultFontSet( frame, loc_id )
...
--	sf:SetVerticalScroll( .0001 )
	...
	if ArkInventory.Global.Location[loc_id_window].drawState <= ArkInventory.Const.Window.Draw.Recalculate then
		...
		sb:SetValue( 0 )
		if c.height > h then
			sb:SetMinMaxValues( 0, c.height - h )
			sb:Show( )
		else
			sb:Hide( )
		end
		...
		if loc_data.isMapped and loc_data.canView then
			...
			if c then
				...
				for bar_id = 1, loc_data.maxBar do
					...
					local obj = _G[string.format( "%s%s%s", c:GetName( ), "Bar", bar_id )]
			...
			if info.equiploc ~= "" then
				...
				if codex.style.slot.upgradeicon.show then
					...
					if PawnShouldItemLinkHaveUpgradeArrowUnbudgeted then
						...
						local itemIsUpgrade = PawnShouldItemLinkHaveUpgradeArrowUnbudgeted( i.h, true )
...
function ArkInventory.ItemTransmogStateAccount( itemID )
	...
	if obj == nil then
		...
		local setID = ArkInventory.CrossClient.GetItemLearnTransmogSet( itemID )
				...
				if frame.ARK_Data.isCompressed then
					...
					if codex.style.slot.itemlevel.stock.show and info.equiploc == "" and info.stock > 0 then
						if codex.style.slot.itemlevel.stock.total then
							...
							local stack_id = frame.ARK_Data.stack_id
							local stack = codex.workpad.compress[stack_id] or { } -- FIX ME - should not have to set this, work out why its not always set
							stock = info.stock * ( stack.total or 1 )
...
function ArkInventory.Frame_Item_PreClick( frame, button, down )
					...
					if IsShiftKeyDown( ) or ( ArkInventory.Global.Mode.Merchant and not IsModifiedClick( ) ) then
						...
						if not ArkInventory.Action.Delete.data.conflict and ArkInventory.db.option.action.delete.enable then
							...
							if ArkInventory.Action.Delete.Check( frame.ARK_Data.blizzard_id, frame.ARK_Data.slot_id, codex ) then
								...
								ArkInventory.OutputDebug( "PreClick-Control: deleting item ", i.h )
	...
	ArkInventory.OutputDebug( "PreClick: end" )
...
--		local bar_id = ArkInventory.CategoryLocationGet( loc_id, cat_id )
	...
	if not usedmycode then
		return ContainerFrameItemButton_OnClick( frame, "LeftButton" )
	end
...
function ArkInventory.Frame_Item_Update_Tint( frame, codex )
	...
	if tinted then
		-- set tint colour after config is updated
		obj:Show( )
	else
		obj:Hide( )
	end
...
function ArkInventory.Frame_Item_OnDragStart_PetJournal( frame )
	...
	if i and i.guid then
		...
		ArkInventory.Collection.Pet.PickupPet( i.guid )
...
function ArkInventory.Frame_Status_Update_Tracking( loc_id_window )
	...
	if codex.style.status.currency.show then
		...
		for i = 1, ( ArkInventory.Const.BLIZZARD.GLOBAL.MAX_WATCHED_TOKENS ) do
...
function ArkInventory.Frame_Changer_OnLoad( frame, loc_id_window )
	...
	local templates = { "ARKINV_TemplateChangerSlotStatic", "ARKINV_TemplateChangerSlotDynamic" }
...
function ArkInventory.Frame_Changer_Update( loc_id_window )
	...
	if loc_id_window == ArkInventory.Const.Location.Bank and ArkInventory.Global.Mode.Bank then
		ArkInventory.CrossClient.UpdateBagSlotStatus( )
	end
...
function ArkInventory.Frame_Changer_Secondary_OnDragStart( frame )
	...
	if InCombatLockdown( ) or ArkInventory.Global.Mode.Edit or ArkInventory.Global.Location[loc_id_window].isOffline or loc_id_window == ArkInventory.Const.Location.Vault then
		return
	end
...
function ArkInventory.Frame_Changer_Slot_Update( loc_id_window, bag_id_window )
	...
	local collapse = true -- FIX ME - add to config - only show first unpurchased tab or all of them
	local display = true
	...
	if ArkInventory.db.option.tooltip.show then
		...
		if false then
			...
			if ArkInventory.Global.Location[loc_id_window].isOffline then
				...
				if bag.count == 0 then
					...
					-- do nothing
					--tooltip:SetText( "empty bag slot", 1.0, 1.0, 1.0 )

				else

					if bag.h then

						tooltip:SetHyperlink( bag.h )

					else

						tooltip:SetText( ArkInventory.Localise["UNKNOWN"], 1.0, 1.0, 1.0 )
		...
		elseif bag.status == ArkInventory.Const.Bag.Status.Purchase then
			...
			if not ArkInventory.Global.Location[loc_id_window].isOffline then
				...
				ArkInventory.TooltipAddNormalLine( tooltip, txt, false )
				tooltip:Show( )
		...
		elseif bag.status == ArkInventory.Const.Bag.Status.Active then
			...
			elseif bag.inv_id then
				...
				tooltip:SetInventoryItem( "player", bag.inv_id )

			end

		elseif bag.status == ArkInventory.Const.Bag.Status.Unknown then

			tooltip:SetText( ArkInventory.Localise["UNKNOWN"] )
...
function ArkInventory.HookBankFrame_ShowPanel( ... )
	...
	ArkInventory.OutputDebug( "HookBankFrame_ShowPanel [", arg1, "] [", arg2, "] [", arg3, "]" )
...
function ArkInventory.HookBankFrame_SetTab( self, ... )
	...
	local arg1, arg2 = ...
	ArkInventory.OutputDebug( "HookBankFrame_SetTab [", arg1, "] [", arg2, "]" )
	...
	-- not really needed yet as changing the bank type also changes the panel tab
...
function ArkInventory.HookBankPanel_SelectTab( self, ... )
	...
	local arg1, arg2 = ...
	ArkInventory.OutputDebug( "HookBankPanel_SelectTab [", arg1, "] [", arg2, "]" )
...
--		ArkInventory:RawHook( "CloseAllBags", "HookCloseAllBags", true ) -- this will cause taint when in combat now, so dont
		...
		if ArkInventory.ClientCheck( ArkInventory.ENUM.EXPANSION.WARWITHIN ) then
			...
			if frame and frame:GetName( ) then
				ArkInventory.MySecureHook( frame:GetName( ), "SelectTab", ArkInventory.HookBankPanel_SelectTab )
			end
