...
do
    ...
    function newWidget(widgetType, props)
		...
		if not newObj then
			newObj = WidgetRegistry[widgetType](props)
			...
			if not newObj.AceGUIWidgetVersion or newObj.AceGUIWidgetVersion < WidgetVersions[widgetType] then
				return newWidget(widgetType,props)
			end
		end
		return newObj
	end
...
function AceGUI:Create(widgetType,props)
	if WidgetRegistry[widgetType] then
		local widget = newWidget(widgetType,props)
